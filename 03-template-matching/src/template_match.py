...
def non_maximal_suppression(
    ...
    for i in range(len(detections_sorted)):
        ...
        current_score, current_bbox = detections_sorted[i]
        final_detections.append((current_score, current_bbox))

        for j in range(i + 1, len(detections_sorted)):
            if suppressed[j]:
                continue

            _, neighbor_bbox = detections_sorted[j]
            iou = bbox_xywh_iou(current_bbox, neighbor_bbox)
...
def find_objects_by_template_matching(
    ...
    """Finds objects in an image by template matching.
    ...
    for lvl in range(levels):
        ...
        score_map = cv.matchTemplate(scaled_image, template, cv.TM_CCOEFF_NORMED)
        score_pyramid.append(score_map)
