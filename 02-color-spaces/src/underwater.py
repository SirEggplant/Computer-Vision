...
_DEFAULT_GAIN_BGR = (0.9, 0.9, 1.3)
_DEFAULT_ALPHA_LUT = 0.7
...
def adjust_gain_bgr(image: np.ndarray, scale: tuple[float, float, float]) -> np.ndarray:
    ...
    b_scaled = np.clip(b * scale[0], 0, 255).astype(np.uint8)
    g_scaled = np.clip(g * scale[1], 0, 255).astype(np.uint8)
    r_scaled = np.clip(r * scale[2], 0, 255).astype(np.uint8)

    return cv.merge([b_scaled, g_scaled, r_scaled])
...
def create_lookup_table(image: np.ndarray, alpha: float) -> np.ndarray:
    ...
    if cdf[-1] > 0:
        cdf_normalized = (cdf - cdf[0]) * 255.0 / (cdf[-1] - cdf[0])
    else:
        cdf_normalized = cdf

    cdf_normalized = np.clip(cdf_normalized, 0, 255).astype(np.uint8)
    partial_lut = (1 - alpha) * identity_lut + alpha * cdf_normalized

    return np.clip(partial_lut, 0, 255).astype(np.uint8)
...
def enhance(
    ...
    adjusted_image = adjust_gain_bgr(image, gain_bgr)
    lab_image = cv.cvtColor(adjusted_image, cv.COLOR_BGR2LAB)
    l_channel, a_channel, b_channel = cv.split(lab_image)
    lut = create_lookup_table(l_channel, alpha_lut)
    enhanced_l_channel = cv.LUT(l_channel, lut)
    enhanced_lab = cv.merge([enhanced_l_channel, a_channel, b_channel])
    output_image = cv.cvtColor(enhanced_lab, cv.COLOR_LAB2BGR)
