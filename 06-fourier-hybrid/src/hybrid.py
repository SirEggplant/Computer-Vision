...
def high_pass(img: np.ndarray, cutoff_freq: float) -> np.ndarray:
    ...
    """
    img_f = np.fft.fft2(img, axes=(0, 1))
    mask = 1.0 - gaussian_low_pass_filter(img.shape[:2], cutoff_freq)
    img_f = img_f * mask
    return np.fft.ifft2(img_f, axes=(0, 1)).real


def hybrid(
        img_low: np.ndarray, img_high: np.ndarray, low_cutoff: float, high_cutoff: float
):
    ...
    """
    low_freq_component = low_pass(img_low, low_cutoff)
    high_freq_component = high_pass(img_high, high_cutoff)

    hybrid_image = low_freq_component + high_freq_component
