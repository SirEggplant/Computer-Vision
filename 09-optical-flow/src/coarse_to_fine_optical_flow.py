...
def warp_by_vector_field(image: np.ndarray, vectors_uv: np.ndarray):
    ...
    map_x = (xs - vectors_uv[..., 0]).astype(np.float32)
    map_y = (ys - vectors_uv[..., 1]).astype(np.float32)

    warped = cv.remap(image, map_x, map_y, interpolation=cv.INTER_LINEAR,
                      borderMode=cv.BORDER_CONSTANT, borderValue=0.0)
    return warped
...
def calculate_gradients(image1: np.ndarray, image2: np.ndarray):
    ...
    Ix = 0.5 * (
        cv.Sobel(I1, cv.CV_32F, 1, 0, ksize=3, scale=0.5) +
        cv.Sobel(I2, cv.CV_32F, 1, 0, ksize=3, scale=0.5)
    )
    Iy = 0.5 * (
        cv.Sobel(I1, cv.CV_32F, 0, 1, ksize=3, scale=0.5) +
        cv.Sobel(I2, cv.CV_32F, 0, 1, ksize=3, scale=0.5)
    )

    It = cv.boxFilter(I2 - I1, ddepth=-1, ksize=(3, 3), normalize=True)
...
def coarse_to_fine_optical_flow(
    ...
    """Calculate the optical flow between two images using the coarse-to-fine approach and the
    Lucas-Kanade method at each level.

    Args:
        image1: The first image, as a (h, w) array.
        image2: The second image, as a (h, w) array.
        levels: The number of pyramid levels to use.
        window_size: The size of the window to use for the Lucas-Kanade method, as an odd integer.
        alpha: The regularization parameter for the optical flow constraint equation.

    Returns:
        flow_uv: The estimated optical flow, as a (h, w, 2) array.
    """
    ...
    for lvl in reversed(range(levels)):
        ...
        if flow is None:
            ...
            prev_h, prev_w = flow.shape[:2]

            flow = cv.pyrUp(flow)
            flow *= 2.0

            if flow.shape[:2] != (h, w):
                flow = cv.resize(flow, (w, h), interpolation=cv.INTER_LINEAR)
                sx = w / prev_w
                sy = h / prev_h
                flow[..., 0] *= sx
                flow[..., 1] *= sy

        I1_warp = warp_by_vector_field(I1, flow)

        Ix, Iy, It = calculate_gradients(I1_warp, I2)

        residual = solve_optical_flow_constraint_equation_lucas_kanade(
            Ix, Iy, It, window_size, alpha
        )

        flow = flow + residual.astype(np.float32)

    scale = np.array(original_size) / np.array(flow.shape[:2])
    flow_uv = scale * cv.resize(flow, original_size[::-1],
                                interpolation=cv.INTER_LINEAR)
    return flow_uv.astype(np.float32)
...
def calculate_optical_flow_pyr_lk_cross_check(
    ...
    """Calculate the optical flow between two images using the coarse-to-fine approach and the
    Lucas-Kanade method at each level. This function also calculates the reverse flow and checks
    for occlusions by comparing the forward and reverse flow.

    If the forward flow and reverse flow disagree by more than `goodness_threshold`, the output
    flow is set to nan at that pixel.
    ...
    Args:
        ...
        window_size: The size of the window to use for the Lucas-Kanade method, as an odd integer.
        alpha: The regularization parameter for the optical flow constraint equation.
        goodness_threshold: The threshold for checking occlusions.

    Returns:
        flow: The estimated optical flow, as a (h, w, 2) array.
    """
    forward_flow = coarse_to_fine_optical_flow(image1, image2,
                                               levels, window_size, alpha)
    reverse_flow = coarse_to_fine_optical_flow(image2, image1,
                                               levels, window_size, alpha)
    ...
    rev_u = cv.remap(
        reverse_flow[..., 0], map_x, map_y,
        interpolation=cv.INTER_LINEAR,
        borderMode=cv.BORDER_REPLICATE
    )
    rev_v = cv.remap(
        reverse_flow[..., 1], map_x, map_y,
        interpolation=cv.INTER_LINEAR,
        borderMode=cv.BORDER_REPLICATE
    )
    rev_at_p = np.dstack((rev_u, rev_v))

    cons = forward_flow + rev_at_p
    cons_norm = np.linalg.norm(cons, axis=2)

    good_mask = cons_norm <= goodness_threshold
    averaged = 0.5 * (forward_flow - rev_at_p)

    flow = np.full_like(forward_flow, np.nan, dtype=np.float32)
    flow[good_mask] = averaged[good_mask]
